...
# NOTE: Run in background thread to avoid blocking boot (Heroku has 60s boot timeout)
Rails.application.config.after_initialize do
  if Rails.env.production?
    # Run in background thread to avoid blocking boot
    Thread.new do
      begin
        # Establish a connection in background thread during boot
        # This triggers schema loading (~20-75 seconds) during dyno startup
